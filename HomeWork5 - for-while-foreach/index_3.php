...
$arr = ['Alex', 'Vanya', 'Tanya', 'Lena', 'Tolya'];
    ...
    for ($i=0;$i<$count; $i++ ) {
     ...
     $arrRes=[];$count=0;
    foreach ( $arr as $key=>$item) $count=$key;
    foreach ( $arr as $key=>$item) $arrRes[$count-$key] = $item;
...
$arr2 = [44, 12, 11, 7, 1, 99, 43, 5, 69];
    ...
    while ($i<count($arr2));
        ...
        $arrRes = [];$count=0;
        foreach ($arr2 as $key => $item) $count = $key;
...
$arrRes=[];$count=0;$countArr=0;
    while ($arr[$countArr]) $countArr++;
    while ($i <= $countArr) {
        if (is_string($arr[$i])) {
            while ($str[$count]) $count++;
            $k=0;$strRes="";
    ...
    $i = 0; $count=0;$strRes="";$arrRes=[];$countArr=0;
    do {$countArr++;} while ($arr[$countArr]);
    do {
        if (is_string($arr[$i])) {
            do {$count++;} while ($str[$count]);
            $k=0;$strRes="";
    ...
    $strRes="";$arrRes=[];$count=0;
    foreach ($arr as $key => $item) {
        if (is_string($item)) {
            $chars = str_split($item);
            foreach ($chars as $key2 => $item2) $count = $key2;
            $k = 0;
            $strRes = "";
            while ($k <= $count) {
                $oneChar = ord($chars[$k]);
...
$k = 0; $countArr=0;$countCycles = 0;$arr=$dataArr;
...
$i=0;
    ...
    while ($i <= ($countArr - 1) ** 2) {
        ...
        if ($k == (count($arr) - 1)) $k = 0;
        $i++;
    }

    $arr=$dataArr; $k = 0; $statusSorted = 0; $countCycles = 0; $i=0;$countArr=0;
    do $countArr++; while ($arr[$countArr])  ;
    do {
        $countCycles++;
        $variable1 = $arr[$k];
